...
class Casino(commands.Cog):
    ...
    async def rps(self, ctx: Context, member: Member = ""):
        ...
        if author_choice == member_choice:
            await ctx.channel.send(
                f"{ctx.author.mention} and {member.mention} got a draw, both chose {author_choice}"
                )
        elif win_dict.get(author_choice) == member_choice:
            await ctx.channel.send(
                f"{ctx.author.mention} ({author_choice}) won against {member.mention} ({member_choice})"
                )
        else:
            await ctx.channel.send(
    ...
    async def blackjack(self, ctx: Context):
        ...
        await ctx.send(
            f"You draw a {add_card(hand)} and a {add_card(hand)}, your total is {blackjacktotal(hand)}"
            )
        ...
        if blackjacktotal(hand) == 21:
            if blackjacktotal(dealer_hand) == 21:
                await ctx.send(
                    f"You got a blackjack, dealers hand was {' '.join(str(card) for card in dealer_hand)} resulting in a draw"
                )
                return
        ...
        while blackjacktotal(hand) < 21:
            ...
            try:
                ...
                if reaction.emoji == "🇭":
                    await ctx.send(
                        f"You draw a {add_card(hand)}, making your total {blackjacktotal(hand)}"
                    )
                if reaction.emoji == "🇶":
                    break
            except asyncio.TimeoutError:
        ...
        await ctx.send(
            f"Dealers face down card is a {dealer_hand[1]}, making their total {blackjacktotal(dealer_hand)}"
        )

        while blackjacktotal(dealer_hand) < 17:
            await ctx.send(
                f"Dealer draws a {add_card(dealer_hand)}, making their total {blackjacktotal(dealer_hand)}"
            )
            if blackjacktotal(dealer_hand) > 21:
        ...
        if blackjacktotal(hand) > blackjacktotal(dealer_hand):
            await ctx.send(
                f"Your total is {blackjacktotal(hand)}, dealers total is {blackjacktotal(dealer_hand)}, you win!"
            )
        elif blackjacktotal(hand) < blackjacktotal(dealer_hand):
            await ctx.send(
                f"Your total is {blackjacktotal(hand)}, dealers total is {blackjacktotal(dealer_hand)}, you lose..."
            )
        else:
            await ctx.send(
